...
const Chat = () => {
      ...
      return messages.filter(
        ({ createdAt, author: { id } }) =>
          ...
          new Date(createdAt).getTime() >
            new Date(channelInfo.lastUpdateAt).getTime()
      ).length
    }
    return 0
  }
  ...
  useEffect(() => {
    return () => {
      setCurrentChannel({
        variables: { channelUrl: `${communityUrl}/general` },
      })
    }
  }, [])
  ...
  return (
    <>
      <S.Container
        {...getRootProps({ onClick: e => e.stopPropagation(), disabled: true })}
      >
        <S.ZoomImage />
