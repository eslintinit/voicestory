...
export default () => {
  ...
  useSubscription(USER_TYPING_STATUS, {
    ...
    onSubscriptionData: ({
      ...
      const userIndex = typingUsers.findIndex(
        (username) => userTypingStatus.username === username
      )
      ...
      if (userTypingStatus.isTyping) {
        ...
        setTypingUsers(
          typingUsers.filter(
            (username) => username !== userTypingStatus.username
          )
        )
      }
    },
