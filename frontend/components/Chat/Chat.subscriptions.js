...
const Subscribe = ({ subscribeToMore, communityUrl, channelUrl }) => {
  ...
  subscribeToMore({
    ...
    updateQuery: (prev, { subscriptionData }) => {
      if (!subscriptionData.data || !subscriptionData.data.deleteMessage)
        return prev
  ...
  subscribeToMore({
    ...
    updateQuery: (prev, { subscriptionData }) => {
      if (!subscriptionData.data || !subscriptionData.data.newReaction)
        return prev
      const { newReaction: reaction } = subscriptionData.data

      const updatedMessageIndex = prev.messages.findIndex(
        ({ id }) => id === reaction.message.id
      )
  ...
  subscribeToMore({
    ...
    updateQuery: (prev, { subscriptionData }) => {
      if (!subscriptionData.data || !subscriptionData.data.updatedReaction)
        return prev
      ...
      const reactionIndex = updatedMessages[
        updatedMessageIndex
      ].reactions.findIndex(({ id }) => id === updatedReaction.id)

      updatedMessages[updatedMessageIndex].reactions.splice(
        reactionIndex,
        1,
        updatedReaction
      )
  ...
  subscribeToMore({
    ...
    updateQuery: (prev, { subscriptionData }) => {
      if (!subscriptionData.data || !subscriptionData.data.removedReaction)
        return prev
